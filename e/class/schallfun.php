...
function SearchAllChangeChar($value){
	global $phome_ecms_charver,$char,$targetchar,$iconv;
...
function LoadSearchAll($lid,$start,$userid,$username){
	...
	$b=0;
	$sql=$empire->query("select id,stb,classid,newstime".$selectf." from {$dbtbpre}ecms_".$lr['tbname']." where id>$start and checked=1 order by id limit ".$line);
...
function ReturnSearchAllSql($add){
	...
	$returnr['keyboard']=htmlspecialchars($keyboard);
